...
describe('SN118 bridging between Base and Subtensor', () => {
    it('approves and bridges the full Base SN118 amount to subnet stake', () => {
        ...
        expect(approval!.transaction.to).toBe(base.contracts.wrappedSn118)
        expect(
            decodeFunctionData({
        ...
        expect(transfer!.label).toContain('SN118')
    })

    it('approves subnet 118 stake in RAO and bridges all of it with zero partner fee', () => {
        ...
        ).toEqual([subtensor.contracts.gateway, SN118_NETUID, amountRao])
        const call = decodeFunctionData({
        ...
        expect(call.args).toEqual([
            base.ccipSelector,
            subtensor.contracts.wrappedSn118,
            recipient,
            0n,
            amountRao,
            amountWei,
    ...
    it('rejects Robinhood SN118 plans in both directions', () => {
        expect(robinhood.contracts.wrappedSn118).toBeNull()
        expect(() =>
            buildEvmToSubtensorPlan({
                ...toFinney,
                evmChain: 'robinhood',
        ...
        ).toThrow('SN118 bridging is not supported on robinhood')
        expect(() =>
            buildSubtensorToEvmPlan({
                ...toBase,
                evmChain: 'robinhood',
    ...
    it('quotes the Base SN118 token and estimates only after its approval exists', async () => {
        ...
        expect(readContract).toHaveBeenCalledWith(
            expect.objectContaining({
                address: base.contracts.wrappedSn118,
    ...
    it('quotes Finney SN118 and checks allowance for subnet 118 before preparing approval', async () => {
        const readContract = vi.fn(async ({ functionName }) =>
        ...
        expect(readContract).toHaveBeenCalledWith(
            expect.objectContaining({
                ...
                functionName: 'allowance',
                args: [sender, subtensor.contracts.gateway, 118n],
            })
        )
    ...
    it('tracks SN118 message IDs in both directions using the existing receipt API', () => {

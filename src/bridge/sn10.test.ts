...
describe('SN10 bridging between Base and Subtensor', () => {
    it('approves and bridges the full Base SN10 amount to subnet stake', () => {
        ...
        expect(approval!.transaction.to).toBe(base.contracts.wrappedSn10)
        expect(
            decodeFunctionData({
        ...
        expect(transfer!.label).toContain('SN10')
    })

    it('approves subnet 10 stake in RAO and bridges all of it with zero partner fee', () => {
        ...
        ).toEqual([subtensor.contracts.gateway, SN10_NETUID, amountRao])
        const call = decodeFunctionData({
    ...
    it('builds Robinhood SN10 plans with its token, gateway and selector', () => {
        ...
        expect(inbound.steps[0]!.transaction.to).toBe(
            robinhood.contracts.wrappedSn10
        )
        ...
        expect(inboundCall.args?.[0]).toBe(robinhood.contracts.wrappedSn10)
        ...
        expect(outboundCall.args?.[1]).toBe(subtensor.contracts.wrappedSn10)
    })

    it('quotes the Base SN10 token and estimates only after its approval exists', async () => {
        ...
        expect(readContract).toHaveBeenCalledWith(
            expect.objectContaining({
                address: base.contracts.wrappedSn10,
    ...
    it('quotes Finney SN10 and checks allowance for subnet 10 before preparing approval', async () => {
        const readContract = vi.fn(async ({ functionName }) =>
        ...
        expect(readContract).toHaveBeenCalledWith(
            expect.objectContaining({
                ...
                functionName: 'allowance',
                args: [sender, subtensor.contracts.gateway, 10n],
            })
        )
    ...
    it('tracks SN10 message IDs in both directions using the existing receipt API', () => {

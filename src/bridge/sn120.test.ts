...
describe('SN120 bridging between Base and Subtensor', () => {
    it('approves and bridges the full Base SN120 amount to subnet stake', () => {
        ...
        expect(approval!.transaction.to).toBe(base.contracts.wrappedSn120)
        expect(
            decodeFunctionData({
        ...
        expect(transfer!.label).toContain('SN120')
    })

    it('approves subnet 120 stake in RAO and bridges all of it with zero partner fee', () => {
        ...
        ).toEqual([subtensor.contracts.gateway, SN120_NETUID, amountRao])
        const call = decodeFunctionData({
        ...
        expect(call.args).toEqual([
            base.ccipSelector,
            subtensor.contracts.wrappedSn120,
            recipient,
            0n,
            amountRao,
            amountWei,
    ...
    it('rejects Robinhood SN120 plans in both directions', () => {
        expect(robinhood.contracts.wrappedSn120).toBeNull()
        expect(() =>
            buildEvmToSubtensorPlan({
                ...toFinney,
                evmChain: 'robinhood',
        ...
        ).toThrow('SN120 bridging is not supported on robinhood')
        expect(() =>
            buildSubtensorToEvmPlan({
                ...toBase,
                evmChain: 'robinhood',
    ...
    it('quotes the Base SN120 token and estimates only after its approval exists', async () => {
        ...
        expect(readContract).toHaveBeenCalledWith(
            expect.objectContaining({
                address: base.contracts.wrappedSn120,
    ...
    it('quotes Finney SN120 and checks allowance for subnet 120 before preparing approval', async () => {
        const readContract = vi.fn(async ({ functionName }) =>
        ...
        expect(readContract).toHaveBeenCalledWith(
            expect.objectContaining({
                ...
                functionName: 'allowance',
                args: [sender, subtensor.contracts.gateway, 120n],
            })
        )
    ...
    it('tracks SN120 message IDs in both directions using the existing receipt API', () => {

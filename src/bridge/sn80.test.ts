...
describe('SN80 bridging between Base and Subtensor', () => {
    it('approves and bridges the full Base SN80 amount to subnet stake', () => {
        ...
        expect(approval!.transaction.to).toBe(base.contracts.wrappedSn80)
        expect(
            decodeFunctionData({
        ...
        expect(transfer!.label).toContain('SN80')
    })

    it('approves subnet 80 stake in RAO and bridges all of it with zero partner fee', () => {
        ...
        ).toEqual([subtensor.contracts.gateway, SN80_NETUID, amountRao])
        const call = decodeFunctionData({
    ...
    it('quotes the Base SN80 token and estimates only after its approval exists', async () => {
        ...
        expect(readContract).toHaveBeenCalledWith(
            expect.objectContaining({
                address: base.contracts.wrappedSn80,
    ...
    it('quotes Finney SN80 and checks allowance for subnet 80 before preparing approval', async () => {
        ...
        expect(readContract).toHaveBeenCalledWith(
            expect.objectContaining({
                ...
                functionName: 'allowance',
                args: [sender, subtensor.contracts.gateway, 80n],
            })
        )
    ...
    it('tracks SN80 message IDs in both directions using the existing receipt API', () => {
